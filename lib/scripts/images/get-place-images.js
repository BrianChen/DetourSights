 ...
 * inserts an Image row, adds a PlaceImage join row, and points the Place at it
 * via the coverImageId FK.
 *
 * Usage:
 ...
 *   node lib/scripts/images/get-place-images.js --destination paris        # all places in destination
 *
 ...
 * Places that already have a coverImageId are skipped — safe to re-run.
 * To force a refresh, clear coverImageId in the DB first.
 */
...
const ONLY_DEST  = destIndex !== -1 ? args[destIndex + 1] : null;
...
async function main() {
  ...
  if (places.length === 0) {
    const hint = ONLY_SLUG ? `slug "${ONLY_SLUG}"` : ONLY_DEST ? `destination "${ONLY_DEST}"` : '';
    console.log(hint ? `No places found for ${hint}` : 'No places found.');
    return;
  }
  ...
  for (const place of places) {
    ...
    try {
      ...
      if (!DRY_RUN) {
        ...
        // Insert into Image (shared metadata), add the PlaceImage join row, then
        // point the Place at its cover Image via the coverImageId FK.
        ...
        await prisma.placeImage.create({
          data: { placeId: place.id, imageId: image.id, position: 0 },
        });
