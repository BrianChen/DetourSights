 ...
 * add-places.js
 *
 * Adds new places to the database. For each entry it:
 *   1. Derives a URL-safe slug from the place name via toSlug() (stored on the Place record)
 *   2. Looks up the destination by toSlug(destinationName) — destination must already exist in DB
 *   3. Checks for an existing place using the composite unique key (destinationId, placeSlug)
...
async function searchPlace(name, destName, destination) {
  ...
  try {
    res = await fetch('https://places.googleapis.com/v1/places:searchText', {
      ...
      headers: {
        ...
        'X-Goog-FieldMask': 'places.formattedAddress,places.location,places.internationalPhoneNumber,places.websiteUri,places.priceLevel,places.regularOpeningHours,places.utcOffsetMinutes,places.reservable,places.accessibilityOptions',
      },
  ...
  // Strip live fields from opening hours before storing; attach utcOffsetMinutes for timezone-aware day highlighting
  let openingHours = null;
  if (place.regularOpeningHours) {
    const { openNow, nextOpenTime, ...stable } = place.regularOpeningHours;
    openingHours = {
      ...stable,
      ...(place.utcOffsetMinutes != null && { utcOffsetMinutes: place.utcOffsetMinutes }),
    };
  }
...
async function main() {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    try {
      ...
      console.log(`    [destination] ✓ ${dest.name} (id: ${dest.id})`);

      // ── Check for existing place via composite key (destinationId, slug) ───
