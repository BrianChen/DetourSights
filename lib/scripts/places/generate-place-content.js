 ...
 *
 * Flow:
 *   1. Look up each entry's destination and place in the database
...
Return this exact JSON structure:
{
  ...
  "howLongToSpend": "string — do not start with a number or time range. Write a sentence first, then give the time. Add context if relevant.",
  "howLongToSpendConfidence": "HIGH | MEDIUM | LOW",
  ...
  "bookInAdvanceWarning": "string or null — only set if bookingRequired is true or there is a strong reason to book ahead. Otherwise null.",
  ...
  await prisma.$transaction(async (tx) => {
    await tx.placeAiGenData.upsert({
      ...
      update: {
        ...
        howLongToSpend:                 data.howLongToSpend                 ?? null,
        howLongToSpendConfidence:       data.howLongToSpendConfidence       ?? null,
...
async function main() {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    try {
      ...
      if (!place) throw new Error(`Place not found in DB (slug: "${slug}") — run add-places.js first`);
