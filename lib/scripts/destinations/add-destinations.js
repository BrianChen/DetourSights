 ...
 * add-destinations.js
 *
 * Adds new destinations to the database and seed file. For each destination it:
 ...
 *   6. Appends the entry to prisma/seed.js and updates the count comment
 *   7. Runs get-destination-images.js to fetch Unsplash photos and upload them to Cloudinary
 *
 * Usage:
 *   node lib/scripts/destinations/add-destinations.js
 *   node lib/scripts/destinations/add-destinations.js --dry-run   # log only, no DB writes
 *
 * Input:
 *   Edit lib/scripts/destinations/destinations-to-add.json before running:
...
async function geocode(name, country) {
  ...
  try {
    ...
    throw new Error(`Google Maps request failed: ${err.message}`);
  } finally {
    clearTimeout(timeout);
  }
...
/**
 * Appends a destination entry to prisma/seed.js and increments the count comment.
 */
function updateSeedFile(name, country, slug, description) {
  let seed = readFileSync(SEED_PATH, 'utf-8');

  // Escape single quotes and backslashes for JS single-quoted string
  const escapedDesc = description.replace(/\\/g, '\\\\').replace(/'/g, "\\'");

  const entry = `    { name: '${name}',`.padEnd(24) +
    ` country: '${country}',`.padEnd(28) +
    ` slug: '${slug}',`.padEnd(28) +
    ` description: '${escapedDesc}' },\n`;

  // Insert before the closing ]; of the destinationData array
  seed = seed.replace(/^  \];$/m, `${entry}  ];`);

  // Increment the count in the header comment
  seed = seed.replace(
    /\/\/ ─── Destinations \((\d+)\)/,
    (_, n) => `// ─── Destinations (${parseInt(n) + 1})`,
  );
...
const GET_IMAGES_PATH = join(__dirname, '../images/get-destination-images.js');

function fetchImages(slug) {
  return new Promise((resolve, reject) => {
    const child = spawn('node', [GET_IMAGES_PATH, '--slug', slug], { stdio: 'inherit' });
    child.on('close', (code) => {
      if (code === 0) resolve();
      else reject(new Error(`get-destination-images.js exited with code ${code}`));
    });
  });
}
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    try {
      ...
      if (!DRY_RUN) {
        ...
        // ── Seed file update ─────────────────────────────────────────────────
        updateSeedFile(name, country, slug, description);
        console.log(`    [seed] ✓ Added to prisma/seed.js`);

        // ── Images ───────────────────────────────────────────────────────────
        console.log(`    [images] Fetching images for ${slug}...`);
        await fetchImages(slug);
      } else {
        console.log('    [db] [dry] Would create destination row');
        console.log('    [seed] [dry] Would append to prisma/seed.js');
        console.log('    [images] [dry] Would run get-destination-images.js');
      }
