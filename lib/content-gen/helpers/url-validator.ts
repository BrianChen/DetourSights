...
function parseIp6(ip: string): number[] | null {
  ...
  // Handle mixed notation (::ffff:127.0.0.1) — convert IPv4 tail to two hex groups
...
// hostname is an ip: https://127.0.0.1/path
...
export async function validateUrl(url: string): Promise<{ safe: boolean; reason?: string }> {
  try {
    ...
    // example: blocks https://google.com:secret@127.0.0.1/admin
    // username: google.com, password: secret
    ...
    // checks string hostname urls by DNS lookup for the ip
